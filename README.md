...
You can visit http://52.87.181.38.xip.io
...
### Step 1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - Check for instance IPv4 public IP - 52.87.181.38
  - we can download a .pem file and connect with following command
    ```
    ssh -i catalog.pem ubuntu@52.87.181.38
    ``` 
...
- To check port 2200 wether working or not by `ssh -i catalog.pem -p 2200 ubuntu@52.87.181.38` 
...
### Step 7: Create an SSH key pair for `grader`
  ...
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : `ssh -i catalog.pem -p 2200 grader@52.87.181.38`.
...
### Step 10: Install and configure PostgreSQL
  ...
  - `CREATE DATABASE coursedata WITH OWNER catalog;`
  - `\c coursedata`
...
`sudo git clone https://github.com/ravitejapittu/CourseCatalog.git`.
...
- We need to change sqlite to postgresql create_engine in `__init__.py`,`database_setup.py` and `lotsofmenus.py`,
   ...
   engine = create_engine('postgresql://catalog:catalog@localhost/coursedata')
   ``` 
...
- Click `Credentials`.
- Create an OAuth Client ID (under the Credentials tab), and add http://52.87.181.38.xip.io and 
http://ec2-52-87-181-38.compute-1.amazonaws.com/ as authorized JavaScript 
origins.
- Add http://52.87.181.38.xip.io/login,http://52.87.181.38.xip.io/gconnect,http://52.87.181.38.xip.io/callback
as authorized redirect URI.
- Download the corresponding JSON file, open it and copy the contents.
- Open `/var/www/catalog/catalog/Gclient_secret.json` and paste the previous contents into the this file.
...
### Step 13.2: Set up and enable a virtual host
  ...
  ```
  <VirtualHost *:80>
      ServerName 52.87.181.38.xip.io
      ServerAlias ec2-52-87-181-38.compute-1.amazonaws.com
      ServerAdmin ubuntu@52.87.181.38
...
- Open your browser to http://52.87.181.38 or http://ec2-52-87-181-38.compute-1.amazonaws.com.
